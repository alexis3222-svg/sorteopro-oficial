    ...
    useEffect(() => {
        ...
        const run = async () => {
            ...
            try {
                ...
                // Wallet (soporta { ok, wallet } o { ok, data })
                if (rWallet && rWallet.ok) {
                    const j = await rWallet.json().catch(() => null);
                    const w = j?.wallet ?? j?.data ?? null;
                    if (j?.ok && w) setWallet(w as WalletRow);
                    else setWallet(null);
                } else {
                    setWallet(null);
                }

                // Movements (soporta { ok, moves } o { ok, movements })
                if (rMoves && rMoves.ok) {
                    const j = await rMoves.json().catch(() => null);
                    const list = j?.moves ?? j?.movements ?? [];
                    if (j?.ok && Array.isArray(list)) setMoves(list as MoveRow[]);
                    else setMoves([]);
                } else {
                    setMoves([]);
                }
